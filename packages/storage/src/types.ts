import { TextileStorageAuth } from '@resource/utils'
import { PathAccessRole, PrivateKey } from '@textile/hub'

export interface iKeyUser {
  identity: PrivateKey
  /**
...
export type NotificationSubscribeListener = (
  data: NotificationSubscribeEventData
) => void

export interface NotificationSubscribeResponse {
  on: (
    type: NotificationSubscribeEventType,
    listener: NotificationSubscribeListener
  ) => void
  /**
   * this function should only be used to listen for the `'done'` event, since the listener would only be called once.
   * or else you could end up having functions leaking (unless you explicitly call the `off()` function).
