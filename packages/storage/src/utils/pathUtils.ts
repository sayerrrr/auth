...
export const isTopLevelPath = (path: string): boolean =>
  sanitizePath(path).split('/').length === 2
...
export const reOrderPathByParents = <T>(
  paths: T[],
  extractor: (path: T) => string
): OrderPathByParentsResult<T> => {
  ...
  return {
    traverseLevels: async (
      cb: (leafs: T[]) => Promise<void>
    ): Promise<void> => {
      const queue = [root]
